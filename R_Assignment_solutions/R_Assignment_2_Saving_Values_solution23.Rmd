---
title: "R Assignment 2 SOLUTIONS"
...
The last one is important. If we saved the mean() of the age variable as the word "mean". We would not be able to use the mean function without a warning until we clear our environment. Sometimes this will break the function and cause an error. 
...
```{r}
# Use the ? operator to bring up the documentation for penelope.  
?penelope
```
...
#You can use the code above to calculate the third quartile and save it as Q3. You only need to change the 0.25 number to the percent that goes with quartile 3. 
Q3 = quantile(x = penelope$weight, probs = 0.75, names = FALSE)
```

3. We need to calculate the IQR also. We have two choices: we can use the built in IQR() function, or simply subtract Q1 from Q3, since we've calculated those values already. Either way we should save the result as IQR_penelope.

```{r}
# calculate and save the IQR. Save the number as IQR_penelope.
IQR_penelope = IQR(penelope$weight, type = 7)
# or
IQR_penelope = Q3-Q1
```
