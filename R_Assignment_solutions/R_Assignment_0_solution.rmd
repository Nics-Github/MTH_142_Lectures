---
title: "R Assignment 0 SOLUTIONS"
...
# This chunk is used to load in the two main libraries we'll be using in this course. To run the chunk press the little green triangle in the upper left hand corner. 
library(tidyverse)
library(openintro)
```
...
The documentation is mediocre for this cars93 dataset. It says price is in dollars, but I think it means 1000s of dollars. Weight is in pounds. 

It also says there are 54 observations in the dataset which means 54 cars represented in the data. Each observation is one row. Each row is information on one car. 

## Part 3 Functions

There are several functions we will use during this course. Most of them are listed on the cheat sheet. You may want to book mark this [website](https://nics-github.github.io/MTH_142_Lectures/MTH_142_Cheat_Sheet_v9.pdf). Let's first look at the mean() function.
...
**Answer:** There are 428 cars in this data. 


2b. How many variables are in this data frame?

**Answer:** There are 19 variables in this data. 
...
```{r}
median(cars04$msrp, na.rm=TRUE)

```

**Answer:** The median price is $27,635.
...
# This is code for a histogram
ggplot(data = cars04)+
  geom_histogram(mapping = aes(x= msrp))
