...
arbuthnot <-  mutate(.data = arbuthnot, total = girls+boys, boy_ratio = boys / total)

present <- mutate(.data = present, total = girls+boys, boy_ratio = boys / total)
```
...
From the babies dataset, make a scatter plot of gestation (explanitory) vs bwt (response). 

Copy the code above and give it a try. 

## Solution: birthweight of babies
...
Find the correlation coefficient for StarWars. Hint: copy and edit the code from above. 
