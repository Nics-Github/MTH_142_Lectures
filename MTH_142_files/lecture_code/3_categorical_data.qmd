...
Once you've loaded the openintro and tidyverse libraries there are many datasets to practice with.
...
```{r}
ggplot(data = avandia, mapping = aes(x = treatment, fill = cardiovascular_problems)) +
  geom_bar(position = "fill")
```
