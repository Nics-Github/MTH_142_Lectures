...
The little bit of code at the top of this document, between some hyphens - - - , is called the yaml. Add your name and the date into the yaml. They should both be strings (that means "in quotes").

## A note on labs

In SDS 220 you will get lots of practice doing statistics through homework assignments, the final project and labs. You will have 7 or 8 labs during this course that will help you do "modern" statistics with simulation and specialized R packages. In this lab the goal is to warm up with some plotting and becoming familiar with R and quarto. You will have 7 questions at the end which will be graded. Finally you will know if you have done your lab correctly if you are able to render without errors. (Getting errors is normal and common. If you are unable to fix them go to office hours or tutoring.) Turn in the rendered .pdf to moodle before the due date. Late labs are not accepted and one will be dropped. Good luck.

## Keep your labs organized

This Lab1.qmd file should be organized into a folder on your computer, ideally in documents (Do not leave it in your downloads folder). All of your labs for this class should go in the same folder. The folders *img* and *more* should be in the same folder. This is important for rendering your work.

## The RStudio Interface

By now you have seen Rstudio and have heard words like: **console**, **quarto**, and **environment pane**. You know about the four main panes of RStudio.[See this site for info on the panes](https://teacherscollege.screenstepslive.com/a/1426910-panes-in-rstudio)
...
You likely have the tidyverse installed already. In the lower right hand corner click on the *Packages* tab. Type the name of each of these packages (tidyverse, openintro) into the search box to see if they have been installed. If these packages do not appear when you type in their name, install them by running the chunk below. 
...
We are choosing to use the tidyverse package because it consists of a set of packages necessary for different aspects of working with data, anything from loading data to wrangling data to visualizing data to analyzing data. Additionally, these packages share common philosophies and are designed to work together. You can find more about the packages in the tidyverse at [tidyverse.org](http://tidyverse.org/). The openintro package has a lot of data we will play with through out the semester.

### Creating a reproducible lab report

We will be using quarto ( or the older rmarkdown) to create reproducible lab reports. The following videos give justification for rmarkdwon, which is similar to quarto.

[**Why use quarto for Lab Reports?**](https://youtu.be/lNWVQ2oxNho)

[**Using quarto for Lab Reports in RStudio**](https://youtu.be/o0h-eVABe9M)

You have already opened Lab_1.qmd and you are working in it now. In the future you can click File -\> Quarto Document to make a new .qmd file.

Going forward you should refrain from typing your code directly in the console, as this makes it very difficult to remember and reproduce the output you want to reference. Potentially the most important feature of quarto files is that they allow for us to nest our `R` code within a written report. In an quarto file, `R` code appears in a gray box, which we call "code chunks." The quarto file knows that the gray box contains `R` code because it begins with three tick marks (\`\`\`), followed by two curly braces that contain a lowercase letter r ({r}). If you do not see these code chunks you may be in *Visual* mode. You can switch to *Source* mode in the upper left.

Instead of typing our `R` code into the console, we encourage you to type any code you produce (final correct answer, or anything you're just trying out) in the `R` code chunk associated with each problem. You can execute the `R` code you type in these code chunks similar to how you typed code into the console and pressed enter/return. Within the code chunk there are two ways to execute a line of `R` code: (1) place your cursor on the line on code and press `Ctrl-Enter` or `Cmd-Enter` at the same time, or (2) place your cursor on the line and press the "Run" button in the upper right hand corner of the quarto file. Alternatively, if you wanted to run all of the `R` code in a given code chunk, you can click on the "Play" button in the upper right hand corner of the code chunk (green sideways triangle).
...
-   placing your cursor on the line and pressing the green "Run" button in the upper right hand corner of the quarto file, or
...
This command does display the data for us, however, printing the whole dataset in the console is not that useful. One advantage of RStudio is that it comes with a built-in data viewer. The *Environment* tab (in the upper right pane) lists the objects in your environment. Clicking on the name `arbuthnot` will open up a *Data Viewer* tab next to your quarto file, which provides an alternative display of the data set. This display should feel similar to viewing data in Excel, where you are able to scroll through the dataset to inspect it. However, unlike Excel, you will not be able to edit the data in this tab. Once you are done viewing the data, You can close this tab by clicking on the `x` in the upper left hand corner.
...
This command above should output the following:
...
arbuthnot <-  mutate(.data = arbuthnot, boy_to_girl_ratio = boys / girls)
```
...
arbuthnot <-  mutate(.data = arbuthnot, boy_ratio = boys / total)
```
...
arbuthnot <-   mutate(.data = arbuthnot, more_boys = boys > girls)
```
...
1.  What command would you use to extract just the counts of girls baptized? Make a new chunk and type it below.
...
2.  Use the plot below to address the following question: Is there an apparent trend in the number of girls baptized over the years? How would you describe it? (To ensure that your lab report is comprehensive, be sure to include the code needed to make the plot as well as your written interpretation.)
...
**Answer:** In general the numbers of girls baptized over the years has gone up, there was a significant dip between 1640 and 1660. Not as many girls were born then. 

3.  Generate a plot of the proportion of boys born over time. What do you see?

```{r }
# This plots a line graph showing the ratio of boys born over time. 
ggplot(data = arbuthnot, aes(x = year, y = boy_ratio)) +
  geom_line()
```

**Answer:** The proportion of boys born overtime hovers above 0.50. We can verify this by finding the mean of the proportions. 
...
**Answer:** The years go from 1940 to 2002. The dimensions are 63x 3 . (63 observations and 3 variables). The variables are `year`, `boys`, `girls`.   
5.  How do these counts compare to Arbuthnot's? Are they of a similar magnitude?

**Answer:** These counts are significantly larger than Arbuthnot's data. We might say they are 3 orders of magnitude larger. (1000 times larger).  

6.  Make a plot that displays the proportion of boys born over time. What do you see? Does Arbuthnot's observation about boys being born in greater proportion than girls hold up in the U.S.? Include the plot in your response. *Hint:* You need to make a column that has the proportion of boys being born the you should be able to reuse your code from Exercise 3 above, just replace the name of the data frame.

```{r}
# The first line of code makes a two new columns (total and boy_ratio) then overwrites the "present" dataframe. 
present <-  mutate(.data= present, total= boys+girls, boy_ratio = boys / total)

# This makes a basic line plot of the new variable created above. 
