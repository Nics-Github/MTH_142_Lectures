---
title: "R Assignment 0 Introduction to R and RStudio"
author: "blank"
date: "Today"
format: pdf
editor: source
...
The little bit of code at the top of this document, between some hyphens - - - , is called the yaml. This has the meta information for the document. You could add author: "Your name" under "title".

## A note on labs

In MTH 142 online you will get lots of practice doing statistics through homework and R assignments. You will have 4 assignments during this course that will help you do "modern" statistics with simulation and specialized R packages. In this R assignment the goal is to warm up with some plotting and becoming familiar with R and quarto. You will have 7 questions at the end which will be peer graded. Which means your peers will grade these so you do not need to include your name in the YAML. Finally you will know if you have done this assignment correctly if you are able to render without errors. (Getting errors is normal and common. If you are unable to fix them go to office hours or or post a screenshot of the error and where it occurred to a forum). Turn in the rendered .pdf to canvas before the due date. Late assignments are not accepted and one will be dropped. Good luck.

## Keep your labs organized

This R_Assignment_0.qmd file should be organized into a folder on HCC's RStudio Workbench or your computer. You can leave it in the same folder you found it. All of your assignments for this class should go in the same folder.
...
# Only click the green arrow if you did not find the tidyverse or openintro package. 
...
We will be using quarto .qmd ( or the older rmarkdown .rmd) to create reproducible lab reports. The following videos give justification for rmarkdwon, which is similar to quarto.
...
You have already opened R_assingment_0.qmd and you are working in it now. In the future you can click File -\> Quarto Document to make a new .qmd file.
...
arbuthnot <-  mutate(.data= arbuthnot, total = boys + girls)
```
...
**Where is the new variable?** When you make changes to variables, like adding a total column, in your dataset, click on the name of the dataset in the environment to update it in the data viewer.
...
1.  Considering the arbuthnot data: What command would you use to extract just the counts of girls baptized? Make a new chunk and type it below.

**Answer**:

2.  Continuing with the `arbuthnot` data, use the plot below to address the following question: Is there an apparent trend in the number of girls baptized over the years? How would you describe it? (To ensure that your lab report is comprehensive, be sure to include the code needed to make the plot as well as your written interpretation.)
...
3.  Generate a plot of the proportion of boys born over time from the `arbuthnot` data. What do you see? (Copy and edit code from above, or use the MTH 142 cheat sheet for ideas.)
...
5.  How do the counts from `present` compare to Arbuthnot's? Are they of a similar magnitude?

**Answer:**

6.  Using `present` make a plot that displays the proportion of boys born over time. What do you see? Does Arbuthnot's observation about boys being born in greater proportion than girls hold up in the U.S.? Include the plot in your response. *Hint:* You need to make a column that has the proportion of boys being born the you should be able to reuse your code from Exercise 3 above, just replace the name of the data frame.
