...
```{r}
Q1 <- fivenum(penelope$weight)[2]
Q3 <- fivenum(penelope$weight)[4]
IQR_weight <- IQR(penelope$weight)
...
```{r}
upper_fence = 2493.75 #put in the upper fence, if there is one or type 10000000 if none. "
lower_fence = -44#put the lower fence if there is one, or 0 if none. 
...
## Check conditions and recalculate the mean.  
...
```{r}
t.test(x= penelope$weight  , mu= 1355 , alternative = "two.sided" )
