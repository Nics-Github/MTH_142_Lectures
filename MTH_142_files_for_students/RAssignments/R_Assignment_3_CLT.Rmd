...
author: "The central limit theorem"
date: "`r Sys.Date()`"
output: pdf_document
execute: 
  echo: true
...
With the penelope data I will demonstrate that the averages from random samples end up looking bell shaped (or normally distributed). All the averages from many samples make up the sampling distribution. This is the key take away from the central limit theorem. Its important because we use the central limit theorem to calculate probabilities that help us understand the population by taking a sample.  
...
```{r}
# This is a graph of all the guesses made. 
...
# This code randomly samples 30 guesses from the penelope weight variable. 
...
The mean of 30 guesses is close to the actual mean of the guessed value which was 1287. If you set a different seed above, by changing the 1 to another number you'll get another mean from a different 30 guesses. 

The central limit theorem says if we find a lot of means of samples sized 30 (or larger), the distribution of those means will be normal. That is, if we make a histogram of a thousand means from a thousand samples it'll look bell shaped. This is because most of the means fall near the actual mean, while less fall far away from it. 

Running the code above a thousand times and keeping track of the means to see this bell shaped graph would be tedious. 
...
Note that the graph above is a distribution of means, generally it is called the sampling distribution. The data distribution, is what the original guesses look like, scroll back to the the first graph to remind yourself the shape of the data distribution. 
...
a_thousand_averages =  penelope |>      
  ...
  rep_sample_n(size = , reps = 1000) |>   # <- Edit the size 
  group_by(replicate)|>               
...
ggplot(data = a_thousand_averages, aes(x = mean)) +
  ...
  labs(
    ...
    y = "number of samples",
    title = "Sampling distribution of the mean",
    subtitle = "Sample size = __, Number of samples = 1000"
  )
```
